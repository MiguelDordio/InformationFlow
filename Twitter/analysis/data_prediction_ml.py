...
def train_test_model(base_folder_path, parameter_optimization):
    ...
    optimized_model = get_optimized_model(ExtraTreesClassifier, parameter_optimization, X_train, y_train)
    train_evaluate_save(optimized_model, X_train, y_train, X_test, y_test)


def standardize(cat_vars_encoded, X_train, X_test):
...
def process_categorical_vars(X_train):
    cat_vars_encoded = get_encoded_cols(X_train)
    for cat in cat_vars_encoded:
        X_train[cat] = X_train[cat].fillna(0)
    return cat_vars_encoded
